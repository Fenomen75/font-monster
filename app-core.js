...
function _buildCardHTML(font, opts){
  ...
  const langHTML=(()=>{
    ...
    return uniq.map((l,i)=>{const c=_LANG_COLORS[i%_LANG_COLORS.length];const sk=l.toLowerCase().replace(/\s+/g,'-');return`<span onclick="event.stopPropagation();filterScript('${sk}')" style="font-size:9px;font-weight:700;padding:2px 7px;border-radius:980px;background:${c.bg};border:1px solid ${c.border};color:${c.text};letter-spacing:.02em;white-space:nowrap;cursor:pointer" title="🌐 Filter by ${l} · see all fonts in this script">${l}</span>`;}).join('')+`<span id="lang-count-${font.id}" style="font-size:9px;font-weight:600;color:var(--text3);white-space:nowrap;margin-left:3px">· .</span>`;
  })();
