...
function renderPvCanvas(){
  ...
  // VACİB: mətnlə yanaşı font-weight və font-family-ni də yeniləyirik, əks halda banner
  // səhifə ilk açılanda təyin olunan qalınlıqda donub qalır və weight seçimi ona təsir etmir.
  const bannerTxt=document.getElementById('heroBannerText');
  if(bannerTxt){
    bannerTxt.textContent=(txt?_sanitize(txt):'')||_pvFamily;
    bannerTxt.style.fontFamily=`'${_pvFamily}',sans-serif`;
    bannerTxt.style.fontWeight=fontWeight;
    bannerTxt.style.fontStyle=fontStyle;
  }
