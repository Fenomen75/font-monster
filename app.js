    ...
    card.innerHTML=`
      <div class="card-header">
        ...
        <div class="card-actions" style="position:relative;z-index:2">
          ...
          <a href="${(font.fontData||font.fontUrl)?'#':`https://fonts.google.com/download?family=${encodeURIComponent(font.name)}`}" ${(font.fontData||font.fontUrl)?'':'target="_blank"'} class="dl-btn"
            onclick="event.stopPropagation();handleDownloadClick('${font.id}','${esc(font.name)}');${(font.fontData||font.fontUrl)?'return false;':''}">
...
function updateNavForUser(user){
  ...
  if(!_isAdmin(user) && user.emailVerified !== true){
    setTimeout(showEmailVerifyBanner, 1500);
  }
}
        ...
        return `<div class="font-card" style="margin-bottom:0;cursor:pointer" onclick="openDetail('${font.id}')">
          <div class="card-header">
            ...
            <div class="card-actions" style="position:relative;z-index:2">
              ...
              <a class="dl-btn" href="https://fonts.google.com/download?family=${encodeURIComponent(font.name)}" target="_blank"
                onclick="event.stopPropagation();handleDownloadClick('${font.id}','${esc(font.name)}')">
...
function openDetail(fontId){
    ...
    <div class="fdp-actions">
      ${(()=>{
        ...
        if(isCommunity){
          ...
          return `<a href="https://fonts.google.com/download?family=${encodeURIComponent(font.name)}" target="_blank" class="fdp-dl"
            onclick="handleDownloadClick('${font.id}','${esc(font.name)}');updateDetailDlCount('${font.id}')">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            Download ZIP
          </a>
  ...
  document.getElementById('fdpDlMainBtn').href=(font.fontData||font.fontUrl)?'#':`https://fonts.google.com/download?family=${encodeURIComponent(font.name)}`;
  if(font.fontData||font.fontUrl){
      ...
      const card = `<div class="font-card" style="margin-bottom:0">
        <div class="card-header" style="cursor:pointer" onclick="openDetail('${sf.id}')">
          ...
          <div class="card-actions" style="position:relative;z-index:2">
            ...
            <a class="dl-btn" href="https://fonts.google.com/download?family=${encodeURIComponent(sf.name)}" target="_blank"
              onclick="event.stopPropagation();handleDownloadClick('${sf.id}','${esc(sf.name)}')" title="Download">
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
              Get
            </a>
          </div>
...
async function submitFont(){
  ...
  if(currentUser.emailVerified === false){
    showToast('⚠ Please verify your email before submitting a font. Check your inbox.');
