...
function switchAdminTab(tab){
  _adminActiveTab=tab;
  ['pending','edits','all','stats','trash','log','export','messages','users','dafont'].forEach(t=>{
  ...
  if(tab==='dafont') _renderAdminDafont();
}
...
function adminApproveEdit(fontId){
  ...
  if(window._fbFns && window._fbDb){
    const {doc, updateDoc} = window._fbFns;
    const fsUpdates = {...updates, editApprovedAt:new Date().toISOString()};
    updateDoc(doc(window._fbDb,'submitted_fonts',fontId),fsUpdates).catch(e=>console.warn('Firestore edit approve error:',e));
  }
  ...
  sendFontStatusEmail({...req, name: req.name+' (Redaktə)'}, 'approved');
}
function adminRejectEdit(fontId,btn){
  ...
  // ?? Email bildirisi gönd?r ??
  sendFontStatusEmail({...rejectedEditFont, name: name+' (Redaktə)'}, 'rejected');
}
...
async function _adminDeleteUser(uid, name){
  if(!uid) return;
  if(!_isAdmin(currentUser)){ showToast('Access denied'); return; }
  var msg = 'DELETE USER\n\nDelete "' + name + '"?\n\nFirestore məlumatları silinəcək.\nSilmədən sonra Firebase Auth səhifəsi avtomatik açılacaq — orada da hesabı sil.\n\nGeri qaytarıla bilməz.';
  ...
  try{
    ...
    // Step 1: deleted_users-a yaz ki doc silinəndən sonra da blok qalsın
    try{ await fns.setDoc(docFn(db,'deleted_users',uid), { deletedAt: new Date().toISOString(), deletedBy: currentUser.id }); }catch(e){ console.warn(e); }
    // Step 2: Ban so user cannot login even if Auth record stays
    try{ await updateDocFn(docFn(db,'users',uid), { banned: true, deletedAt: new Date().toISOString() }); }catch(e){ console.warn(e); }
    // Step 3: Delete Firestore user doc
    ...
    _renderAdminUsers();
    // Open Firebase Auth console so admin can delete the Auth account too
    setTimeout(function(){
      window.open('https://console.firebase.google.com/project/fontan-7bae4/authentication/users','_blank');
    }, 500);
    showToast('✅ Silindi! Firebase Auth səhifəsi açıldı — orada da "' + name + '" i tapıb sil (3 nöqtə → Delete account).');
  }catch(err){
...
// CSS VAR PANEL
function toggleCssVarPanel(){
  const body=document.getElementById('cssVarBody'),arrow=document.getElementById('cssVarArrow');
  const open=body.style.display==='none';body.style.display=open?'flex':'none';
  arrow.textContent=open?'?':'?';if(open)updateCssVarPreview();
}
function updateCssVarPreview(){
  if(!currentDetailFont)return;
  const el=document.getElementById('cssVarPreviewEl');
  el.style.cssText=`font-family:'${currentDetailFont.name}',sans-serif;font-size:${document.getElementById('cv-size').value||'1rem'};line-height:${document.getElementById('cv-lh').value||'1.5'};letter-spacing:${document.getElementById('cv-ls').value||'0em'};color:${document.getElementById('cv-color').value||'#111'};padding:10px 0;border-top:1px solid var(--border);margin-top:4px;word-break:break-word`;
}
function copyCssVars(){
  if(!currentDetailFont)return;
  const css=`:root {\n  --font-family: '${currentDetailFont.name}', sans-serif;\n  --font-weight: ${activeDetailWeight};\n  --font-size: ${document.getElementById('cv-size').value||'1rem'};\n  --line-height: ${document.getElementById('cv-lh').value||'1.5'};\n  --letter-spacing: ${document.getElementById('cv-ls').value||'0em'};\n  --text-color: ${document.getElementById('cv-color').value||'#111111'};\n}`;
...
function openDetail(fontId){
      ...
      ${(()=>{
        ...
        if(isCommunity){
          ...
          return `<a href="#" class="fdp-dl"
            onclick="handleDownloadClick('${font.id}','${esc(font.name)}');updateDetailDlCount('${font.id}');return false;">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            Download ZIP
          </a>
  ...
  // CSS var panel
  document.getElementById('cssVarBody').style.display='none';document.getElementById('cssVarArrow').textContent='?';
  ...
  if(font.fontData||font.fontUrl){
    ...
    document.getElementById('fdpDlMainBtn').setAttribute('target','_blank');
    ...
    document.getElementById('fdpDlMainBtn').onclick=()=>{handleDownloadClick(font.id,font.name);updateDetailDlCount(font.id);};
  }
  ...
  // Deterministik sıralama: eyni font hər dəfə eyni "similar" fontları göstərsin
  function _seededSort(arr, seed){
    let s=seed|0;
    return arr.slice().sort(()=>{ s=(s*1664525+1013904223)&0xffffffff; return (s>>>16)/65536 - 0.5; });
  }
  const _seed = font.id.split('').reduce((a,c)=>((a<<5)-a+c.charCodeAt(0))|0,0);
  const otherFonts = _seededSort(FONTS.filter(f => f.id !== font.id && f.cat === font.cat), _seed);
  const fallback = _seededSort(FONTS.filter(f => f.id !== font.id && f.cat !== font.cat), _seed+1);
  const pool = [...otherFonts, ...fallback];
      ...
      const card = `<div class="font-card" style="margin-bottom:0">
        <div class="card-header" style="cursor:pointer" onclick="openDetail('${sf.id}')">
          ...
          <div class="card-actions" style="position:relative;z-index:2">
            ...
            <a class="dl-btn" href="#"
              onclick="event.stopPropagation();handleDownloadClick('${sf.id}','${esc(sf.name)}');return false;" title="Download">
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
              Get
            </a>
          </div>
...
function openSubmit(){
  ...
  const _fbAuthUser = window._fbAuth && window._fbAuth.currentUser;
  const _isVerified = _fbAuthUser ? _fbAuthUser.emailVerified === true : currentUser.emailVerified === true;
  if(!_isVerified){
